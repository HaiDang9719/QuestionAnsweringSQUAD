...
def init_from_checkpoint(model_config, global_vars=False):
  tvars = tf.global_variables() if global_vars else tf.trainable_variables()
  initialized_variable_names = {}
  scaffold_fn = None
  if model_config['init_checkpoint'] is not None:
    if model_config['init_checkpoint'].endswith("latest"):
      ckpt_dir = os.path.dirname(model_config['init_checkpoint'])
...
def get_train_op(model_config, total_loss, grads_and_vars=None):
  global_step = tf.train.get_or_create_global_step()

  # increase the learning rate linearly
  if model_config['warmup_steps'] > 0:
    warmup_lr = (tf.cast(global_step, tf.float32)
                 / tf.cast(model_config['warmup_steps'], tf.float32)
                 * model_config['learning_rate'])
  else:
    warmup_lr = 0.0

  # decay the learning rate
  if model_config['decay_method'] == "poly":
    decay_lr = tf.train.polynomial_decay(
        model_config['learning_rate'],
        global_step=global_step - model_config['warmup_steps'],
        decay_steps=model_config['train_steps'] - model_config['warmup_steps'],
        end_learning_rate=model_config['learning_rate'] * model_config['min_lr_ratio'])
  elif model_config['decay_method'] == "cos":
    ...
    raise ValueError(model_config['decay_method'])

  learning_rate = tf.where(global_step < model_config['warmup_steps'],
                           warmup_lr, decay_lr)

  if (model_config['weight_decay'] > 0 and not model_config['use_tpu'] and
      model_config['num_core_per_host'] > 1):
  ...
  if model_config['weight_decay'] == 0:
    ...
    optimizer = AdamWeightDecayOptimizer(
        ...
        weight_decay_rate=model_config['weight_decay'])
  ...
  clipped, gnorm = tf.clip_by_global_norm(gradients, model_config['clip'])

  if getattr(model_config, "lr_layer_decay_rate", 1.0) != 1.0:
    n_layer = 0
    ...
    for i in range(len(clipped)):
      for l in range(n_layer):
        if "model/transformer/layer_{}/".format(l) in variables[i].name:
          abs_rate = model_config['lr_layer_decay_rate'] ** (n_layer - 1 - l)
          clipped[i] *= abs_rate
