...
def get_qa_outputs(model_config, features, is_training):
  ...
  with tf.variable_scope("end_logits"):
    if is_training:
      ...
      start_top_log_probs, start_top_index = tf.nn.top_k(
          start_log_probs, k=model_config['start_n_top'])
      ...
      end_input = tf.tile(output[:, :, None],
                          [1, 1, model_config['start_n_top'], 1])
      ...
      end_logits = tf.layers.dense(
          ...
          kernel_initializer=initializer,
          name="dense_1")
      end_logits = tf.reshape(end_logits, [seq_len, -1, model_config['start_n_top']])
      end_logits = tf.transpose(end_logits, [1, 2, 0])
      end_logits_masked = end_logits * (
          1 - p_mask[:, None]) - 1e30 * p_mask[:, None]
      end_log_probs = tf.nn.log_softmax(end_logits_masked, -1)
      end_top_log_probs, end_top_index = tf.nn.top_k(
          end_log_probs, k=model_config['end_n_top'])
      end_top_log_probs = tf.reshape(
          end_top_log_probs,
          [-1, model_config['start_n_top'] * model_config['end_n_top']])
  ...
  with tf.variable_scope("answer_class"):
    ...
    ans_feature = tf.layers.dropout(ans_feature, model_config['dropout'],
